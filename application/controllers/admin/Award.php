...
class Award extends Admin_Controller
{
    ...
    public function awardList($user_id = null, $flag = null)
    {
        if ($this->input->is_ajax_request()) {
            $this->load->model('datatables');
            $this->datatables->table = 'tbl_employee_award';
            $this->datatables->column_order = array('award_name', 'user_id', 'gift_item', 'award_amount', 'award_date', 'given_date');
            $this->datatables->column_search = array('award_name', 'user_id', 'gift_item', 'award_amount', 'award_date', 'given_date');
            $this->datatables->order = array('employee_award_id' => 'desc');

            $fetch_data = $this->datatables->get_employee_award_by_id($user_id, $flag);
            ...
            foreach ($fetch_data as $_key => $v_award_info) {
                ...
                $action = null;
                $sub_array = array();
                $sub_array[] = $v_award_info->employment_id;
                $sub_array[] = $v_award_info->fullname;
